...
/*   Updated: 2022/09/19 14:03:50 by lloison          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	verify_first_line(char *buf, char *valid_chars)
{
	...
	if (buf[i] >= 32 && buf[i] <= 126)
	{
		...
		return (-1);
	return (verify_first_line2(buf, valid_chars, &i));
}

int	verify_first_line2(char *buf, char *valid_chars, int *i)
{
	if ((buf[*i] >= 32 && buf[*i] <= 126) || buf[*i] != valid_chars[0])
	{
		valid_chars[1] = buf[*i];
		*i = *i + 1;
	}
		...
		return (-1);
	if ((buf[*i] >= 32 && buf[*i] <= 126)
		|| buf[*i] != valid_chars[0] || buf[*i] != valid_chars[1])
		*i = *i + 1;
	else 
		return (-1);
	if (buf[*i] != 0)
