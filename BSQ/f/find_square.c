...
/*   Updated: 2022/09/19 18:34:56 by lloison          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	disp_matrice(t_map **map, int size)
{
	for(int i = 0; i < (*map)->height; i++)
	{
		for(int j = 0; j < (*map)->length; j++)
		{
			if ((*map)->matrice[i][j] <= size - 1 && (*map)->matrice[i][j] >= size - 3)
				printf("\033[0;33m");
			if ((*map)->matrice[i][j] == size)
				printf("\033[1;31m");
