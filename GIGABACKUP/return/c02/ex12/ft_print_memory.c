...
void	ft_ch_m(unsigned char	*addr, int sz)
{
	int	i;
...
void	ft_pr_m(unsigned char hexarr[17], unsigned char *addr, int sz, int i)
{
	while (i < 17)
	{
		ft_putchar(hexarr[i]);
...
void	ft_hex_mem(unsigned long add, unsigned char	*addr, int sz, int i)
{
	char			*basehex;
	unsigned char	hexarr[17];
